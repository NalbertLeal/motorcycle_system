...
def _deserialize_image(data) -> Tuple[bytes, str, int]:
    ...
    return bbox, message.frame.frame_number, frame, message.bbox.plate_text
...
def consume_frames(data):
    value = data.value
    bbox, frame_number, frame, plate_text  = _deserialize_image(value) # plate text
    ...
    cv2.imshow(plate_text, resized_frame) # plate text
